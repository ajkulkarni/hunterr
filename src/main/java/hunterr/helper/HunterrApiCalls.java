...
public class HunterrApiCalls {
	...
	public static Job[] searchJobs(String city, String company, String title, String jobType, int days) {
		...
		String response = client.target("http://api.indeed.com/ads").path("apisearch")
				.queryParam("publisher", "2529823375038480").queryParam("l", city)
				.queryParam("q", "company:" + company + " " + "title:" + title).queryParam("jt", jobType)
				.queryParam("fromage", days).queryParam("v", "2").queryParam("format", "json").queryParam("limit", 100)
